/*
    ...
    Cliente: Senai-Timbó
    Turma:AI PSII
    Data:09/07/24-Versão: 1.5
    

	Resumo: Confecção de um portão automático por distância de um sensor.
    		O sensor usado foi um HC-SR04 e de motor, um micro servo.
*/
...
#define pinServo 6					//Define o pino do servo
...
#define pingOut 2 					//Define uma porta do servo
#define pingInp 3					//Define uma porta do servo 
#define LED_A 4						//Define a porta do LED A
#define LED_B 5						//Define a porta do LED B
#define PinMoto 10					//Define o pino do motor
#define pinBt	7					//Define o pino relacionado ao botão
...
int delay_reset = 500;
bool statusBt = LOW;				//Variavel para o status do botão
...
void setup() {						//Função Setup
  ...
  pinMode(pinBt, INPUT);			//O pino do botão será de entrada de sinal
  
}

void loop() {
  									// Configura o pino como saída para enviar o pulso
  pinMode(pingOut, OUTPUT);			//Define a porta para a saída do som				
  digitalWrite(pingOut, HIGH);		//Envia um pulso alto para a posta do sensor
  delayMicroseconds(5);				//Delay de 10 microssegundos
  digitalWrite(pingOut, LOW);		//Reseta o sensor

  									
  
  duration = pulseIn(pingInp, HIGH);//Mede o tempo até que a porta fique HIGH novamente

  
  distance = duration *0.01715;		// Calcula a distância em centímetros

  statusBt = digitalRead(pinBt);
  Serial.print("Status: ");
  ...
  else if(distance<disancia_Atv){		//Se a distância for menor que o estabelecido
    abrirPortao();
